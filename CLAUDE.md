...
14. **`UPDATE.bat` Logdatei bei `%~dp0`** — Das Installationslog (`install-autostart.js install`) landet unter `%~dp0update.log` (= Verzeichnis der UPDATE.bat), **nicht** in `%TEMP%`. Grund: `%TEMP%` zeigt im Admin-Kontext auf `C:\Windows\Temp`, nicht auf den Nutzer-Temp-Ordner. Die Logdatei wird weder bei Erfolg noch bei Fehler gelöscht. Schritt 5 prüft zusätzlich per `tasklist /FI "IMAGENAME eq node.exe"` ob `node.exe` nach dem Start tatsächlich läuft — schlägt auch diese Prüfung fehl, wird `[FEHLER]` mit Log-Inhalt und `pause` angezeigt statt fälschlicherweise „UPDATE ERFOLGREICH!".
